...
class TestCloud(unittest.TestCase):
    ...
    @mock.patch('foundationallm.cli.core.cloud.get_custom_clouds', lambda: [])
    def test_add_get_delete_custom_cloud(self):
        ...
        with mock.patch('foundationallm.cli.core.cloud.CLOUD_CONFIG_FILE', tempfile.mkstemp()[1]) as\
                config_file:
            with mock.patch('foundationallm.cli.core.cloud.get_custom_clouds', lambda: []):
                add_cloud(cli, c)
            ...
            with mock.patch('foundationallm.cli.core.cloud._get_cloud', lambda _, _1: c):
    ...
    def test_get_default_latest_profile(self):
        with mock.patch('foundationallm.cli.core.cloud.CLOUD_CONFIG_FILE', tempfile.mkstemp()[1]):
            cli = DummyCli()
    ...
    def test_modify_known_cloud(self):
        with mock.patch('foundationallm.cli.core.cloud.CLOUD_CONFIG_FILE', tempfile.mkstemp()[1]) as config_file:
            cli = DummyCli()
            config = configparser.ConfigParser()
    ...
    @mock.patch('foundationallm.cli.core.cloud._set_active_subscription', autospec=True)
