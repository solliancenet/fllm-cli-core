...
EXTENSIONS_MOD_PREFIX = 'fllmext_'

AZEXT_METADATA_FILENAME = 'fllmext_metadata.json'

EXT_METADATA_MINCLICOREVERSION = 'fllmext.minCliCoreVersion'
EXT_METADATA_MAXCLICOREVERSION = 'fllmext.maxCliCoreVersion'
EXT_METADATA_ISPREVIEW = 'fllmext.isPreview'
EXT_METADATA_ISEXPERIMENTAL = 'fllmext.isExperimental'

# If this FLLM CLI core version has breaking changes that do not support older versions of extensions,
...
class ExtensionNotInstalledException(Exception):
    ...
    def __str__(self):
        return f"The extension {self.extension_name} is not installed. Please install the extension via `fllm extension add -n {self.extension_name}`."
...
class WheelExtension(Extension):
    ...
    def get_all():
        ...
        # Sort the extensions by name to support overwrite extension feature: https://github.com/solliancenet/foundationallm/issues/25782.
