...
class AAZList(AAZBaseValue):
    ...
    def __getitem__(self, idx) -> AAZBaseValue:
        ...
        try:
            val = item_schema._ValueCls(item_schema, self._data[idx])
        except Exception as ex:
            from ._field_type import AAZDictType
            item_schema = AAZDictType()
            item_schema.Element = AAZDictType()
            val = AAZDict(item_schema, None)
            val._data = {}
            #val._schema[0] = AAZDictType()
            val._data[0] = self._data[idx]
