...
These credentials implement foundationallm.core.credentials.TokenCredential by exposing `get_token` method for Track 2
SDK invocation.
...
foundationallm.cli.core.auth.util.AccessToken to build the return value. See below credentials as examples.
"""
...
class UserCredential(PublicClientApplication):
    ...
    def __init__(self, client_id, username, **kwargs):
        ...
        if len(accounts) > 1:
            raise CLIError(f"Found multiple accounts with the same username '{username}': {accounts}\n"
                           "Please report to us via Github: https://github.com/solliancenet/foundationallm/issues/20168")

        if not accounts:
            raise CLIError("User '{}' does not exist in MSAL token cache. Run `fllm login`.".format(username))
    ...
    def get_token(self, *scopes, claims=None, **kwargs):
        ...
        self.http_client._http_client.headers['Origin'] = 'fllm'
        ...
        try:
            ...
            # launch browser, but show the error message and `fllm login` command instead.
            else:
                raise
