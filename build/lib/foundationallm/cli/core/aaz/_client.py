...
from foundationallm.cli.core.cloud import (CloudEndpointNotSetException, CloudSuffixNotSetException,
...
class AAZClientConfiguration(Configuration):
    ...
    def _configure(
        ...
        from foundationallm.core.pipeline.policies import UserAgentPolicy, HeadersPolicy, ProxyPolicy, \
...
class AAZBaseClient(PipelineClient):
    ...
    def build_lro_polling(self, no_wait, initial_session, deserialization_callback, error_callback,
        ...
        from foundationallm.core.polling.base_polling import OperationResourcePolling
    ...
    def get_cloud_endpoint(cls, ctx, arm_idx):
        from foundationallm.cli.core.cloud import CloudEndpoints
        endpoints = None
    ...
    def get_cloud_suffix(cls, ctx, arm_idx):
        from foundationallm.cli.core.cloud import CloudSuffixes
        # retrieve by indexes mapping
        suffixes = None
    ...
    def _retrieve_value_in_arm_cloud_metadata(ctx, arm_idx):
        from foundationallm.cli.core.cloud import retrieve_arm_cloud_metadata, get_active_cloud_name
...
@register_client("FllmClient")
class AAZFllmClient(AAZBaseClient):
    """FLLM Client for Management APIs"""
    ...
    def _build_configuration(cls, ctx, credential, **kwargs):
        ...
        return AAZClientConfiguration(
            credential=credential,
            credential_scopes=resource_to_scopes(ctx.cli_ctx.cloud.endpoints.management_client_id),
            **kwargs
        )
